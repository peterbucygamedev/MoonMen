...
func _on_body_entered(body):
	if body.is_in_group("players") and body.velocity.x <= 0:
		body.position = Vector2(right_area.position.x - 105, body.position.y)
	...
	if body.is_in_group("power_crate") and body.linear_velocity.x <= 0:
		...
		c.transform = Transform2D(body.rotation, body.scale, body.skew, Vector2(right_area.position.x - 125, body.position.y))
